...
contract Base is Test {
    ...
    function setUp() public {
        ...
        weth = new ERC20Mock();
        weth.mint(user, 10000000 * 1 ether);

        wbtc = new ERC20Mock();
        wbtc.mint(user, 10000000 * 1 ether);

        usdc = new ERC20Mock();
        usdc.mint(user, 10000000 * 1 ether);

        usdt = new ERC20Mock();
        usdt.mint(user, 10000000 * 1 ether);

        glm = new ERC20Mock();
        glm.mint(user, 10000000 * 1 ether);

        depositor.setAllowedToken(address(weth), true);
        depositor.setAllowedToken(address(wbtc), true);
        depositor.setAllowedToken(address(usdc), true);
        depositor.setAllowedToken(address(usdt), true);
        depositor.setAllowedToken(address(glm), true);
        ...
        usdcMockAggregator = new MockV3Aggregator(8, 3000e8);
        usdtMockAggregator = new MockV3Aggregator(8, 3000e8);
        glmMockAggregator = new MockV3Aggregator(8, 3000e8);
        ...
        mainRouter.addAllowedToken(chainSelector, address(usdc), address(usdcMockAggregator), 18);
        mainRouter.addAllowedToken(chainSelector, address(usdt), address(usdtMockAggregator), 18);
        mainRouter.addAllowedToken(chainSelector, address(glm), address(glmMockAggregator), 18);
    }
    ...
    modifier DepositAndMint (ERC20Mock _token, uint256 _amountDeposit, uint256 _amountMint) {
        ...
        assert(_amountMint == gm.balanceOf(user));
    ...
    function test_mint() external Deposit(weth, 1 ether) {
        ...
        console.log("Balance of user minted before minting: ", gm.balanceOf(user));
        ...
        console.log("Balance of user minted after minting: ", gm.balanceOf(user));
    ...
    function test_burn() external DepositAndMint(weth, 1 ether, 1000 ether){
        ...
        vm.startPrank(user);
        gm.approve(address(minter), _amountBurn);
        minter.burn(_amountBurn);
        vm.stopPrank();

        console.log("Balance of user minted after burning: ", gm.balanceOf(user));

        assert(_amountMint - _amountBurn == gm.balanceOf(user));
    ...
    function testAddMoreThanOneCollateral() external Deposit(weth, 1 ether) Deposit(wbtc, 1 ether) Deposit(usdc, 1 ether) Deposit(usdt, 1 ether) Deposit(glm, 1 ether) {
        ...
        assert(mainRouter.getUserCollateralValue(user, chainSelector, address(usdc)) == 3000 ether);
        assert(mainRouter.getUserCollateralValue(user, chainSelector, address(usdt)) == 3000 ether);
        assert(mainRouter.getUserCollateralValue(user, chainSelector, address(glm)) == 3000 ether);
    ...
    function testBurnAndMint() external DepositAndMint(weth, 1 ether, 1000 ether) {
        vm.startPrank(user);
        gm.approve(address(minter), 500 ether);
    ...
    function testLiquidate() external DepositAndMint(weth, 1 ether, 1000 ether) {
        ...
        assert(_amountMint == gm.balanceOf(liquidator));
        ...
        vm.startPrank(liquidator);
        gm.approve(address(minter), 1000 ether);
