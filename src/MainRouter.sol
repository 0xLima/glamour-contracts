...
contract MainRouter is CCIPBase, FunctionsBase {
    ...
    address private ethereumDepositor;
    address private ethereumMinter;
    ...
    modifier onlyEthereumDepositor(address _caller) {
        if (ethereumDepositor != _caller){
            revert NotAllowed();
        }
        _;
    }

    modifier onlyEthereumMinter(address _caller) {
        if (ethereumMinter != _caller){
            revert NotAllowed();
        }
        _;
    }

    function setEthereumDepositor(address _depositorContract) external onlyOwner{
        ethereumDepositor = _depositorContract;
    }

    function setEthereumMinter(address _minterContract) external onlyOwner {
        ethereumMinter = _minterContract;
    }
    ...
    )   external onlyEthereumDepositor(msg.sender) {
    ...
    function burnAndMint(
        ...
        uint64 _destinationChainSelector,
        address _receiver
    ) external onlyEthereumMinter(msg.sender) {
        _burn(_burner, _chainSelector, _amount);
        _mint(_destinationChainSelector, _receiver, _burner, _amount);
    }

    function deposit(address _depositor, uint64 _sourceChainSelector, address _token, uint256 _amount) external onlyEthereumDepositor(msg.sender) {
        _deposit(_depositor, _sourceChainSelector, _token, _amount);
    }

    function burn(address _burner, uint64 _sourceChainSelector, uint256 _amount) external onlyEthereumMinter(msg.sender) {
    ...
    function getEthereumDepositor() public view returns (address) {
        return ethereumDepositor;
    }

    function getEthereumMinter() public view returns (address) {
        return ethereumMinter;
    }
